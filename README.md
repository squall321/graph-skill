...
- **143 그래프 타입 / 8 엔진 패밀리(+linked-view 버스) / 19 분류 카테고리**
- 엔진: `xy-core`(Canvas 2D, 94 타입·플러그인 25종, grouped/stacked 막대·콤보·T1/T2 정형 판정그림 포함) · `field-core` · `polar-core` · `smith-core`
...
pytest tests/ -q            # 442+ tests (수치 검증·게이트·결정성·실브라우저 시각·웹/MCP 서빙; optional deps 없으면 일부 skip)
